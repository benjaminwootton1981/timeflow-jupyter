...
//git env vars
env.git_url = 'git@github.com:benjaminwootton1981/timeflow-jupyter.git'
env.git_branch = '*/master'
//slack env vars
env.slack_url = 'https://timeflow-workspace.slack.com/services/hooks/jenkins-ci?token=SWyKhNnHgdygWRjMLkhEU4Fc'
env.notification_channel = 'timeflow-infra-alerts'
//jenkins env vars
env.jenkins_server_url = 'http://54.194.74.126/'
...
pipeline {
    ...
    stages {
        ...
        stage('BUILD'){
            stages {
                stage('Start Build'){
                    steps {
                        script {
                            notifySlack("Timeflow Jupyter job is started to build! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
                        }
                    }
                }
                ...
                stage('Build Image') {
                    steps {
                        sh '''
                            docker build -t ${ECR_URI}/${PROJECT_NAME}:${VERSION} .
                        '''
                    }
                }
                stage('Push Image') {
                ...
                stage('End Build'){
                    steps {
                        script {
                            notifySlack("Timeflow Jupyter job is completed BUILD PHASE! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
                        }
                    }
                }
            }
        ...
        stage ('DEPLOY') {
            steps {
                build job: 'TimeFlow-Infratructure', parameters: [
        ...
        //             steps {
        //                 script {
        //                     notifySlack("Timeflow Admin GUI job is validating service! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
        //                 }
        //             }
        //         }
        ...
        //             steps {
        //                 script {
        //                     notifySlack("Timeflow Admin GUI job is completed VALIDATE PHASE! Build logs from jenkins server $jenkins_server_url/job/$JOB_NAME/$BUILD_NUMBER/console", notification_channel)
        //                 }
        //             }
        //         }
        //     }
